  ...
  stages {
    stage('Build & Test') {
      ...
      environment {
        ...
        NPM_REG = 'http://host.docker.internal:8081/repository/npm-group/'
        NPM_STRICT_SSL = 'false'
      }
      steps {
        withCredentials([usernamePassword(credentialsId: 'nexus-npm', usernameVariable: 'NEXUS_USER', passwordVariable: 'NEXUS_PASS')]) {
          sh '''#!/usr/bin/env bash
            set -euxo pipefail
    ...
    registry=${NPM_REG}
    always-auth=true
    //${NPM_REG#http://}:_auth=$(printf '%s' "${NEXUS_USER}:${NEXUS_PASS}" | base64 | tr -d '\\n')
    ...
    EOF
            ...
            if grep -q "registry.npmjs.org" package-lock.json 2>/dev/null; then
    ...
    stage('Docker build & push (frontend)') {
      ...
      steps {
        withCredentials([
          usernamePassword(credentialsId: 'nexus-npm',    usernameVariable: 'NPM_USER',    passwordVariable: 'NPM_PASS'),
          usernamePassword(credentialsId: 'nexus-docker', usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')
        ]) {
          sh '''#!/usr/bin/env bash
            set -euxo pipefail

            AUTH="$(printf '%s' "${NPM_USER}:${NPM_PASS}" | base64 | tr -d '\n')"
            cat > .npmrc <<EOF
    registry=${NPM_REG}
    always-auth=true
    //${NPM_REG#http://}:_auth=${AUTH}
    strict-ssl=${NPM_STRICT_SSL}
    EOF
