...
test('should return error when user registers twice for the same event', async ({page}) => {
  ...
  await page.getByRole('spinbutton', {name: 'Capacity'}).fill('2');
  ...
  const toast = page.getByTestId('toast').filter({ hasText: 'You are already registered for this event.' }).last();
